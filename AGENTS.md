...
No `flake.nix`, Makefile, or justfile exists. This is a plain Go module — use `go` directly. CI (`.github/workflows/ci.yml`) runs 8 jobs on every push and PR (plus a weekly scheduled full run): a Go compatibility test matrix (go.mod toolchain + previous release; `-race`; coverage + Codecov upload gated on the `CODECOV_TOKEN` secret, pinned entry only; the scheduled run also executes the strict-timing contract pass at `TimingScale: 3` via `GO_IDEMPOTENCY_CONTRACT_TIMING_SCALE`), `go vet`, `golangci-lint`, a `gofmt` check, a `go mod tidy` diff check, 3-minute fuzzing per fuzz target (all four: the three root store targets plus middleware dispatch), `govulncheck`, and a docs job (stale-refs guard + lychee link check, SHA-pinned action). The lychee scan covers CHANGELOG compare links, which is why RELEASING.md pushes master and the tag in one push (the links 404 until the tag exists); it excludes historical snapshots (`docs/feedback`, `docs/status`, `docs/planning`).
...
- **The contract suite never swallows Store errors.** A backend error inside the suite is itself a contract violation and must fail the run (`mustSeen` helper, explicit `t.Fatalf`s) — swallowing one would let the zero value masquerade as a passing assertion and certify a broken backend. Sabotage wrappers in the negative tests are value-based and return nil errors, so this does not interfere with the harness's output matching.
...
- **Error sentinels are declared with the `error` interface type** (`var ErrDuplicate error = errorfamily.NewConflict(...)`), never the concrete `*errorfamily.Error`: the sentinel is the domain contract callers match via `errors.Is`; the classification type is an implementation detail that must stay swappable.
- **go-error-family errors are terminal.** They are already classified (family, code, retryability) and are returned as-is, never wrapped; the `github.com/larsartmann/go-error-family.New` constructor signatures are exempted in `wrapcheck.ignore-sigs` (`.golangci.yml`) for exactly this reason.
- **`fmt.Errorf` with `%w` is the enforced wrap inside `middleware/`** (ADR-002: stdlib-only package, so neither samber/oops nor go-error-family may be imported there). Everywhere else, construct errors with go-error-family.
- **`erraudit` (hierarchical-error audit) project policy is `--enforce-go-error-family`**, not the tool's `--enforce-samber-oops` default — run `erraudit . --enforce-go-error-family` (expect 0 violations) after touching error handling.
