...
- **`contract/`** — reusable contract test suite verifying any `Store` implementation against 13 invariants: `RunTests` (fast defaults) and `RunTestsStrict(t, factory, Options{TimingScale})` for slow CI runners. Consumers import it to verify their own backend.
- **`contract/contract_negative_test.go`** — 14 deliberately broken Stores proving the suite fails against every invariant's violation and names it.
...
- **`internal/teststore/`** — module-internal test Store shared by the contract self-test, the negative tests, and the middleware tests. Consumers cannot import it; never add production-backend code here.
...
- **Package-level test suites** (same conventions): `contract/contract_test.go` (suite self-test against `internal/teststore`), `contract/contract_negative_test.go` (detection proofs), `middleware/middleware_test.go` + `middleware/http_test.go` (unit + 50-goroutine at-most-once), `middleware/fuzz_test.go` (`FuzzDispatch`), `middleware/example_test.go` (`ExampleNewCommand`), `example/example_test.go` (contract suite over the demo backend).
- **Contract invariants pair with negative tests.** Every new invariant in `contract/contract.go` must ship with a broken-Store scenario in `contract_negative_test.go` and a README invariant-table row in the same PR (see "Extending the suite" in `contract.go`).
