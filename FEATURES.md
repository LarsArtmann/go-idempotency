...
| **Proven detection (negative tests)** — 14 deliberately broken Stores, at least one per contract invariant; the suite is proven to fail against each violation and name it        | `contract/contract_negative_test.go`                                    |
