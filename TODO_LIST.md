...
- [ ] **Add `Store` interface contract test** — table-driven test suite that any `Store` implementation must pass. Run against `MemoryStore` now; consumers reuse it to verify their own backend implementations.
