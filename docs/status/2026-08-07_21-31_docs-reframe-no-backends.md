...
3. **No implementation example was provided.** The user said "make it clear that the end user implements." I made the *claim* clear in every doc, but didn't provide the *evidence* — a code example showing how simple it is to implement the interface. The message would land much harder with a 15-line Redis example. This is a "show, don't tell" gap.
