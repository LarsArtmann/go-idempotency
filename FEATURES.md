...
| **Fuzz tests** — `FuzzCheckAndRecord` and `FuzzRecord` verify panic-safety and invariants on arbitrary keys/TTLs; includes a Close-during-concurrent-ops race test              | `fuzz_test.go`                                                          |
